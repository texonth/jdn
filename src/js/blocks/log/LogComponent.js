...
class LogComponent extends React.Component {
	...
	displayLogIcon = () => {
		...
		if (icon) {
			return (
				<img className={classes.logIcon} src={icon}/>
			)
		}
		return (
			<p>Empty Log</p>
	...
	render () {
		...
		return (
			...
			>
				...
				{mainModel.showLog && <div>{
					mainModel.applicationLog.map((item, index) => <div key={item.message + index}>{this.displayLogItem(item)}</div>)
				}</div>}
			</div>
		)
	}
