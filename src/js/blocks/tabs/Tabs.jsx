  ...
  render() {
    ...
    return (
      <nav
        className={
          direction
            ? `UnderlineNav UnderlineNav--${direction}`
            : "UnderlineNav UnderlineNav"
        }
      >
        <div
          className="UnderlineNav-body"
          style={direction ? { flexDirection: "row-reverse" } : {}}
        >
          {[0, 1, 2].map((index) => {
            ...
            return (
              <a
                ...
                title={tab}
                key={tab + index}
                className={cn("UnderlineNav-item", {
                  selected: index === mainModel.currentTab,
                })}
                onClick={() => {
...
class LeftBlock extends React.Component {
  render() {
    ...
    return <div className={classes.leftBlock}>{children}</div>;
  }
}
...
class RightBlock extends React.Component {
  render() {
    const { classes, children } = this.props;
    return <div className={classes.rightBlock}>{children}</div>;
  }
}
